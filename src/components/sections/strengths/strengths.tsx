        ...
        <div className="w-full bg-white mt-4 rounded-2xl p-4 select-none">
          ...
          <div className="relative flex justify-between items-center mb-8">
            {!openRename && !openEditMenu && (
              ...
              >
                {formData.strengths.title || "Strengths"}
              </span>
            )}
          ...
          {formData.strengths.items.map((item) => (
            ...
            >
              <div className="w-full flex flex-col gap-y-1 select-none text-sm text-[#7D8588] mt-1 bg-transparent outline-none py-3 pl-3 pr-12 border-[1px] border-[#AFB4B5] transition-all duration-300 rounded">
                <span className="font-semibold">
                  {item.strength || "Your Strengths"}
                </span>
                <span>{item.explain || "Explain how it benefits your work."}</span>
              </div>
              <div
                onClick={(event) => {
                  setEditingItemId(item.id);
          ...
          >
            ...
            <div className="w-full">
              <div
                onClick={() => {
                  if (editingItemId) {
                    removeSectionItem("strengths", editingItemId);
                  }
                  setopenMobileEdit(false);
                }}
                className="w-full p-6 text-[#505A5D] flex items-center gap-2 border-b-[1px] border-b-[#E4E4E4] pb-4"
        ...
        <form className="w-full px-6 bg-[#FAFBFD]">
          ...
          <div className="w-full">
            ...
            <input
             value={
                editingItemId
                  ? formData.strengths.items.find(
                      (item) => item.id === editingItemId
                    )?.strength || ""
                  : formData.strengths.strength
              }
              onChange={(event) => {
                if (editingItemId) {
                  handleSectionChange(
